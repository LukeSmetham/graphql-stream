...
describe('getOrCreateUser Resolver', () => {
    ...
    test('makes a POST request to the /user endpoint with the get_or_create parameter', () => {
        ...
        resolver.resolve(resolveParams).then(response => {
            ...
            expect(response.url).toStrictEqual(`https://api.stream-io-api.com/api/v1.0/user?api_key=${credentials.api_key}&get_or_create=true`);
            expect(response.data).toStrictEqual(resolveParams.args);
        });
    });
